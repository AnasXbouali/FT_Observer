...
As in Feedback 1, the reference trajectory uses the target infection
level computed from the estimated initial conditions
...
For Feedback 2, the feedback laws themselves use the constant lower
bound

```math
I_l=
\frac{1-S_h+S_h\log S_h}
{1+\beta S_h Q}.
```
...
The first one is the NSN feedback control with the constant threshold
$I_l$:
...
\begin{cases}
1-\dfrac{S_h}{\hat S},
& \text{if } \hat I \geq I_l \text{ and } \hat S>S_h,\\[6pt]
0, & \text{otherwise}.
\end{cases}
```

The second feedback is a modified version with an additional
multiplicative term:

```math
\widetilde u_{2}(\hat I,\hat S):=
\begin{cases}
1-\dfrac{S_h}{\hat S}\dfrac{I_l}{\hat I},
& \begin{aligned}
  \text{if } \hat I \geq I_l \\
  \text{and } \hat S > S_h,
\end{aligned} \\[8pt]
0, & \text{otherwise}.
\end{cases}
...
The additional term $I_l/\hat I$ in $\widetilde u_2$ amplifies the
control value when $\hat I$ exceeds the threshold, forcing the infected
trajectory to remain closer to the level $I_l$ while consuming more
budget.
...
The closed-loop trajectories obtained with $\widetilde u_2$ are added
